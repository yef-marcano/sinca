$(document).ready(function() {
    ...
    $('#tblAlmacen').dataTable({
        ...
        "ajax": "../controller/php/ac_listalmacen.php"
    });

    $('#tblAlmacen').removeClass('display').addClass('table table-striped table-bordered');

    $('#btn_add').click(function() {
        if (xajax.call('validAction', {parameters: [8, 1], mode: 'synchronous'}) == 0) {
            showMessage(7);
            return;
        }
        cleanForm();
        $('#labelFormAlmacen').html("Agregar Almacen");
        $('#sideBarFormAlmacen').sidebar('show');
...
$('#btn_save').css({visibility:"visible"});
    ...
    $('#btn_cancel').click(function() {
        ...
        $('#sideBarFormAlmacen').sidebar('hide');
    });

    $('#tabsAlmacen .item').tab({history: false});
...
function cleanForm() {
    $('#tabsAlmacen .item').tab('change tab', 'first');
    $("#hdn_id").val("");
    $("#txt_descripcion").val("");
...
function save() {
    xajax_save(xajax.getFormValues('frmAlmacen'));
}

function viewData(id) {
    if (xajax.call('validAction', {parameters: [8, 2], mode: 'synchronous'}) == 0) {
        showMessage(7);
        return;
    }
    cleanForm();
    $('#divLoader').removeClass("disabled").addClass("active");
    $('#labelFormAlmacen').html("Ver Almacen");
    $('#sideBarFormAlmacen').sidebar('show');
...
function editData(id) {
    if (xajax.call('validAction', {parameters: [8, 3], mode: 'synchronous'}) == 0) {
        showMessage(7);
        return;
    }
    cleanForm();
    $('#divLoader').removeClass("disabled").addClass("active");
    $('#labelFormAlmacen').html("Editar Almacen");
    $('#sideBarFormAlmacen').sidebar('show');
//    $('#btn_save').removeClass("hidden");
$('#btn_save').css({visibility:"visible"});
    xajax.call('view', {parameters: [id], mode: 'synchronous'});
...
function deleteData(id) {
    if (xajax.call('validAction', {parameters: [8, 4], mode: 'synchronous'}) == 0) {
        showMessage(7);
        return;
    }
