$(document).ready(function() {
    ...
    xajax.call('findValuesDropDownSolicitud_construpatria', {parameters: [0], mode: 'synchronous'});
    ...
    $("#txt_proyecto").change(function () {          
        xajax_findValuesDropDownSolicitud_construpatria($("#txt_proyecto").val());
    });  
    
    $("#txt_solicitud_construpatria").change(function () {          
        xajax_cargarSolicitud($("#txt_solicitud_construpatria").val());
    });              
    ...
    $('#btn_add').click(function() {
        if (xajax.call('validAction', {parameters: [25, 1], mode: 'synchronous'}) == 0) {
            showMessage(7);
            return;
        }
        cleanForm();
        $('#labelFormOrden_salida_construpatria').html("Agregar Orden Salida Construpatria");
        $('#sideBarFormOrden_salida_construpatria').sidebar('show');
    ...
    $('#btn_cancel').click(function() {
        ...
        $('#sideBarFormOrden_salida_construpatria').sidebar('hide');
    });

    $('#tabsOrden_salida_construpatria .item').tab({history: false});
    ...
    var rules = {
        ...
        txt_solicitud_construpatria: {
            identifier: 'txt_solicitud_construpatria',
            rules: [{
                    type: 'empty',
        ...
        txt_proyecto: {
            ...
            rules: [{
                    type: 'empty',
                    prompt: 'Por favor Ingrese un Proyecto'
                }]
        },
        txt_almacen: {
...
function save() {
    xajax_save(xajax.getFormValues('frmOrden_salida_construpatria_planilla'));
}
...
function view(view) {
    ...
    if ($('#hdn_id').val() != "") {
        ...
        document.getElementById("txt_solicitud_construpatria").disabled=true;
        }
}
...
function editData(id) {
    ...
    $('#labelFormOrden_salida_construpatria').html("Editar Orden salida Construpatria");
    $('#sideBarFormOrden_salida_construpatria').sidebar('show');
    //$('#btn_save').removeClass("hidden");
    $('#btn_save').css({visibility:"visible"});
