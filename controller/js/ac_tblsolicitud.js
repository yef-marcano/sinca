...
function aproveData(id) {
    ...
    cargar('frm_solicitud_planilla_aprobe.php?solicitud=' + id + '&view=0', '', '',19);
}



function denegaData(id) {
    if (xajax.call('validAction', {parameters: [19, 6], mode: 'synchronous'}) == 0) {
        showMessage(7);
        return;
    }
    if (confirm("Seguro desea Negar La Solicitud con el id " + id + "?")) {
        xajax_denegaData(id);
    }
}
function printData(id) {
    ...
    window.open('../reportes/ficha_reporte.php?id='+id,'_blank','');
}

//clvestatus_solicitud