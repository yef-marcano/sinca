...
//        $('#btn_save').removeClass("hidden");
    ...
    var settings = {
        ...
        onFailure: function() {
            ...
            $('html,body').animate({
                scrollTop: $("#frmSolicitudPlanilla").offset().top
            }, 1);
        }
...
function view(view) {
    if (view == 1) {        
        ...
        $('#tabsSolicitud .item').tab('change tab', 'first');
        $('#labelFormSolicitud').html("Ver Area");
    $('#sideBarFormSolicitud').sidebar('show');
    }
    cleanForm();
    ...
    if ($('#hdn_id').val() != "") {
        ...
        $("[id*=btn_eliminar_]").hide();
         $('#DropDownProyecto').dropdown('destroy');
...
function insertTr() {
    ...
    var tdFiltro = "<td><select class='validationCustom' id='txt_unidad_despachada_" + num + "' name='txt_unidad_despachada[]'  onchange='cargarUnidad(" + num + ")'></select></td>";

    var tr = "<tr id='tr_" + num + "'>" + tdInsumo + tdCantidad +  tdButton +  tdFiltro +"</tr>";
    ...
    xajax.call('findValuesDropDownUnidad_despachada', {parameters: ["txt_unidad_despachada_" + num], mode: 'synchronous'});
