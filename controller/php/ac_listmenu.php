...
$subqueryIdCreacion= "(SELECT CONCAT_WS(' ', tblusuario.strnombre, tblusuario.strapellido) FROM tblusuario WHERE tblusuario.clvcodigo= tblmenu.clvusuario_creador)";
$subqueryIdModulo= "(SELECT tblmodulo.strdescripcion from tblmodulo where tblmodulo.clvcodigo=tblmenu.clvmodulo)";
//$subqueryFechaCreacion= "DATE_FORMAT(fecha_creacion, '%d-%m-%Y %h:%i %p')";

$columns = array(
    array('db' => 'clvcodigo', 'dt' => 0),
    array('db' => 'strdescripcion', 'dt' => 1),
    array('db' => 'strarchivo', 'dt' => 2),
    array('db' => $subqueryIdModulo, 'dt' => 3, 'alias' => 'nombre_modulo'),    
    array('db' => 'intorden', 'dt' => 4),    
