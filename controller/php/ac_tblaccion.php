...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblAccion').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormAccion').sidebar('hide');");
        ...
        logError("tblaccion", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblaccion", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblaccion", 'delete', $e->getMessage());
    }
    return $objResponse;
}
