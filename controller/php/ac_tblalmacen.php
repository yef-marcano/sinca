...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblAlmacen').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormAlmacen').sidebar('hide');");
        ...
        logError("tblalmacen", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblalmacen", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblalmacen", 'delete', $e->getMessage());
    }
    return $objResponse;
}
