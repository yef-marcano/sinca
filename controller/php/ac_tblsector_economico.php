...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblSector_economico').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormSector_economico').sidebar('hide');");
        ...
        logError("tblsector_economico", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblsector_economico", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblsector_economico", 'delete', $e->getMessage());
    }
    return $objResponse;
}
