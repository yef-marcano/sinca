...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblTipo_proyecto').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormTipo_proyecto').sidebar('hide');");
        ...
        logError("tbltipo_proyecto", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tbltipo_proyecto", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tbltipo_proyecto", 'delete', $e->getMessage());
    }
    return $objResponse;
}
