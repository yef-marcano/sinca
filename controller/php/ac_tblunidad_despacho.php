...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblUnidad_despacho').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormUnidad_despacho').sidebar('hide');");
        ...
        logError("tblunidad_despacho", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblunidad_despacho", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblunidad_despacho", 'delete', $e->getMessage());
    }
    return $objResponse;
}
