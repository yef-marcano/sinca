...
function save($form) {
    ...
    try {
        if ($objTable->clvcodigo == '') {
            ...
            $objResponse->script("$('#tblUnidad_medida').dataTable()._fnAjaxUpdate();");
            $objResponse->script("$('#sideBarFormUnidad_medida').sidebar('hide');");
        ...
        logError("tblunidad_medida", $action, $e->getMessage());
    }
    return $objResponse;
}
...
function view($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblunidad_medida", "view", $e->getMessage());
    }
    return $objResponse;
}
...
function delete($id) {
    ...
    try {
        ...
        $objResponse->script("showMessage(6)");
        logError("tblunidad_medida", 'delete', $e->getMessage());
    }
    return $objResponse;
}
