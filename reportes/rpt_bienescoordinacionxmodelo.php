...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,$tipo.' POR COORDINACIÓN',0,0,'C');
		$this->Ln(10);
...
and g.stritemb='COORDINACION' and b.id_tipo_maestro=".$_REQUEST['id']."       
...
if ($data){
    ...
    $pdf->Cell(0, 10,'Coordinación: '.utf8_decode($data[0]["gerencia"]), 0, 0, 'L');
	$pdf->Ln();
	...
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["gerencia"]){
			...
			$pdf->SetDrawColor(0);
			$pdf->SetLineWidth(0.3);
			$pdf->Cell(0, 10,'Coordinación: '.utf8_decode($data[$i]["gerencia"]), 0, 0, 'L');
            $pdf->Ln();
            $pdf->SetFillColor(255,0,0);
			$pdf->SetTextColor(255,255,255);
			$pdf->SetDrawColor(0);
