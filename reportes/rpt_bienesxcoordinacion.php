...
and g.stritemb='COORDINACION' and b.id_estatus_maestro in (86,88,89,90,91,618,619)      
ORDER BY id_gerencia_maestro,id_ubicacion,responsable,b.id_ubicacion";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){
	...
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["gerencia"] || $ubicacion!=$data[$i]["id_ubicacion"]){
			...
			and b.bolborrado=0 and t.bolborrado=0 and g.stritemb='COORDINACION' and b.id_estatus_maestro in (86,88,89,90,91,618,619) and g.stritemc='".$gerencia."' and b.id_ubicacion=".$ubicacion." group by t.stritema, b.id_tipo_maestro, b.id_gerencia_maestro, g.stritemc, b.id_estatus_maestro
			...
			$pdf->SetDrawColor(0);
			$pdf->SetLineWidth(.3);       
			$pdf->SetWidths(array(50,15,15,20,25,20));
	        $pdf->SetAligns(array('C','C','C','C','C','C'));
			$pdf->Row(array('Equipo','Asignado',utf8_decode('Da̱ado'),'Extraviado','Desincorporado','Total'));
			...
			if ($data1){	
				...
				for ($j= 0; $j < count($data1); $j++){
					if ($equipo==$data1[$j]['equipo']){			
						...
						$totalc=$asignado + $danado + $extraviado + $desincorporado;
						$ttotalc+=$totalc;
						$pdf->Row(array(utf8_decode($equipo),$asignado,$danado,$extraviado,$desincorporado,$totalc));	
				...
				$ttotalc+=$totalc;
				$pdf->Row(array(utf8_decode($equipo),$asignado,$danado,$extraviado,$desincorporado,$totalc));	
				$pdf->Row(array('TOTAL',$tasig,$tdan,$text,$tdes,$ttotalc));
			}
