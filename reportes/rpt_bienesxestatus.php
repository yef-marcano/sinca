...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,'BIENES TELEMÁTICOS POR ESTATUS',0,0,'C');
		$this->Ln(10);
...
ORDER BY id_tipo_maestro, id_estatus_maestro";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){	
    $gerencia=$data[0]["tipo"];
    $pdf->SetFont('Arial','B',8);
	$pdf->SetFillColor(255,255,255);
	$pdf->SetTextColor(0);
	$pdf->SetDrawColor(0);
	$pdf->SetLineWidth(0.3);
    $pdf->Cell(0, 10,'Equipo: '.utf8_decode($data[0]["tipo"]), 0, 0, 'L');
	$pdf->Ln();
	$pdf->SetFillColor(255,0,0);
	$pdf->SetTextColor(255,255,255);
	$pdf->SetDrawColor(0);
    $pdf->SetWidths(array(36,36,36,26,46,36,36));
	$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C','C'));
	$pdf->Row(array('MARCA','MODELO','SERIAL','ESTATUS','GERENCIA','USUARIO','UBICACIÓN'));
	...
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["tipo"]){
			...
			$pdf->SetDrawColor(0);
			$pdf->SetLineWidth(0.3);
			$pdf->Cell(0, 10,'Equipo: '.utf8_decode($data[$i]["tipo"]), 0, 0, 'L');
            $pdf->Ln();
            $pdf->SetFillColor(255,0,0);
			$pdf->SetTextColor(255,255,255);
			$pdf->SetDrawColor(0);
            ...
            $pdf->Row(array($data[$i]["marca"], $data[$i]["modelo"],utf8_decode($data[$i]["strserial"]), utf8_decode($data[$i]["estatus"]),utf8_decode($data[$i]['gerencia']),$responsable,utf8_decode($data[$i]["ubicacion"])));
			$total++;
		}
		else
		{
