...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,'BIENES TELEMÁTICOS POR GERENCIA',0,0,'C');
		$this->Ln(10);
...
if ($data){
	...
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["gerencia"] || $ubicacion!=$data[$i]["id_unidad_maestro"]){
			...
			$pdf->Row(array('Equipo','Disponible','Asignado',utf8_decode('DaÃ±ado'),'Extraviado','Desincorporado','Robado','Reemplazado','Total'));
