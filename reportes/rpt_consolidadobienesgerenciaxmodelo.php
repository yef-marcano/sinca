...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,'CONSOLIDADO DE '.utf8_decode($tipo).' DE LAS GERENCIAS POR MODELOS',0,0,'C');
		$this->Ln(10);
		...
		$this->SetDrawColor(0);
		$this->SetLineWidth(.3);       
		$this->SetWidths(array(20,12,12,12,12,15,15,15,15,15,15,15,15,15,15,15,12,15));
        $this->SetAligns(array('C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C'));
		$this->Row(array('Modelo','PRE','AI','OPP','CJ','OAF','ORRHH','OSTI','OGC','OAC','GG','GFPC','GEPAS','GIS','GC','CN','EXT','Total'));
	}
...
and b.id_tipo_maestro=".$_REQUEST['id']." and b.bolborrado=0 and t.bolborrado=0 and m.bolborrado=0 and g.bolborrado=0 and g.stritemb='GERENCIA' and b.id_estatus_maestro in (86,87,88)
...
$pdf->SetWidths(array(20,12,12,12,12,15,15,15,15,15,15,15,15,15,15,15,12,15));
...
if ($data){
	...
	$idmodelo=$data[0]['id_modelo_maestro'];
	$tpre=0;
	$tai=0;
	$topp=0;
	$tcj=0;
	...
	$sql="select count(b.*), t.stritema as equipo, m.stritema as modelo, b.id_modelo_maestro
	from tblbienes b, tblmaestros t, tblmaestros m
	where b.id_tipo_maestro=t.id_maestro
	and b.id_estatus_maestro in (86,87,88,89) and b.id_ubicacion=689
	and b.id_modelo_maestro=".$idmodelo."
	and b.id_gerencia_maestro=17 and b.id_modelo_maestro=m.id_maestro
	and b.bolborrado=0 and t.bolborrado=0 and m.bolborrado=0
	group by t.stritema, m.stritema, b.id_modelo_maestro";
	$conn= new Conexion();
	$conn->abrirConexion();
	$conn->sql=$sql;
	...
	for ($i= 0; $i < count($data); $i++){
		if ($equipo==$data[$i]['modelo']){			
			...
			$total=$pre+$ai+$opp+$cj+$oaf+$orrhh+$osti+$ogc+$oac+$gg+$gfpc+$gepas+$gis+$gc+$cn+$ext;	
			$ttotal+=$total;
			$pdf->Row(array(utf8_decode($equipo),$pre,$ai,$opp,$cj,$oaf,$orrhh,$osti,$ogc,$oac,$gg,$gfpc,$gepas,$gis,$gc,$cn,$ext,$total));	
			...
			$idmodelo=$data[$i]['id_modelo_maestro'];
			$sql="select count(b.*), t.stritema as equipo, m.stritema as modelo, b.id_modelo_maestro
			from tblbienes b, tblmaestros t, tblmaestros m
			where b.id_tipo_maestro=t.id_maestro
			and b.id_estatus_maestro in (86,87,88) and b.id_ubicacion=689
			and b.id_modelo_maestro=".$idmodelo."
			and b.id_gerencia_maestro=17 and b.id_modelo_maestro=m.id_maestro
			and b.bolborrado=0 and t.bolborrado=0 and m.bolborrado=0
			group by t.stritema, m.stritema, b.id_modelo_maestro";
	...
	$total=$pre+$ai+$opp+$cj+$oaf+$orrhh+$osti+$ogc+$oac+$gg+$gfpc+$gepas+$gis+$gc+$ext+$cn;	
	$ttotal+=$total;
	$pdf->Row(array(utf8_decode($equipo),$pre,$ai,$opp,$cj,$oaf,$orrhh,$osti,$ogc,$oac,$gg,$gfpc,$gepas,$gis,$gc,$cn,$ext,$total));		
	$pdf->Row(array('TOTAL',$tpre,$tai,$topp,$tcj,$toaf,$torrhh,$tosti,$togc,$toac,$tgg,$tgfpc,$tgepas,$tgis,$tgc,$tcn,$text,$ttotal));
