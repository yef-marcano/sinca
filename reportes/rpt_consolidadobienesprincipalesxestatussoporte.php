...
if ($data){
	...
	for ($i= 0; $i < count($data); $i++){
		if ($equipo==$data[$i]['equipo']){			
			...
			$total=$disponible + $prestamo + $asignado + $danado + $extraviado + $desincorporado + $robado;
			$ttotal+=$total;
			$totalc=$disponiblec + $prestamoc + $asignadoc + $danadoc + $extraviadoc + $desincorporadoc + $robadoc;
			$ttotalc+=$totalc;
			$ttotalt=$total+$totalc;
			
			
			//$pdf->Row(array(utf8_decode($equipo),($asignado*100)/($asignado+$asignadoc),($asignadoc*100)/($asignado+$asignadoc),($danado*100)/($danado+$danadoc),($danadoc*100)/($danadoc+$danado),($desincorporado*100)/($desincorporado+$desincorporadoc),($desincorporadoc*100)/($desincorporado+$desincorporadoc),($disponible*100)/($disponible+$disponiblec),($disponiblec*100)/($disponible+$disponiblec),($extraviado*100)/($extraviadoc+$extraviado),($extraviadoc*100)/($extraviado+$extraviadoc),($prestamo*100)/($prestamo+$prestamoc),($prestamoc*100)/($prestamo+$prestamoc),($robado*100)/($robado+$robadoc),($robadoc*100)/($robado+$robadoc),($reemplazado*100)/($reemplazado+$reemplazadoc),($reemplazadoc*100)/($reemplazado+$reemplazadoc),($total*100)/($total+$totalc),($totalc*100)/($totalc+$total)));	
