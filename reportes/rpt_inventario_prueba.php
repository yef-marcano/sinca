...
class PDF  extends PDF_MC_Table
{
	...
	function Footer()
	{
		...
		$this->Cell(172,10,utf8_decode('Generado a través del Sistema SICET en Fecha '.date('d/m/Y').' por el Usuario '.rtrim($_SESSION["strnombrefenix"]).' '.rtrim($_SESSION["strapellidofenix"]).'  |  Fuente: OSTI - '.date('Y').'  |  Licencia: GPL/GNU'),0,0,'L');
                $this->Cell(20,10,utf8_decode('Página '.$pagina.'/{nb}'),0,0,'R');
...
and t.bolborrado=0 and m.bolborrado=0 and e.bolborrado=0 and mo.bolborrado=0  and u.bolborrado=0 and b.id_estatus_maestro in (86,88) and b.id_ubicacion in (3,4,680,681)
AND b.id_gerencia_maestro='".$_REQUEST['gerencia']."'";
    $sql.=" and b.id_tipo_maestro<>132";

$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
$data=$conn->ejecutarSentencia(2);
$equipos=$data[0]['total'];
...
$pdf->MultiCell(0,7,utf8_decode('Por medio de la presente hoy, a los '.fechaLetra(date('d/m/Y')).', se hace constar que '.trim($osti[0]['strnombre']).' '.trim($osti[0]['strapellido']).', titular de la C.I.- V-'.number_format(trim($osti[0]['strcedula']),0,",",".").', en su carácter de '.$osti[0]['cargo'].', hace entrega a '.trim($usu[0]['strnombre']).' '.trim($usu[0]['strapellido']).', titular de la C.I.- V-'.number_format(trim($usu[0]['strcedula']),0,",",".").', quien se desempeña como '.$usu[0]['cargo'].', los equipos de computación que se encuentran asignados a la '.$usu[0]['gerencia'].'.'),0,'J',0);
$pdf->Ln(10);
$pdf->MultiCell(0,7,utf8_decode('Se anexa la lista identificada con el N° '.$_REQUEST['correlativo'].' contentiva de '.$equipos.' equipos de computación indicando la Descripción, N° de Seriales y Usuarios responsables de los mismos.'),0,'J',0);
$pdf->Ln(10);
...
$pdf->SetX(35);
$pdf->Cell(58,5,utf8_decode(trim($osti[0]['strnombre'])).' '.utf8_decode(trim($osti[0]['strapellido'])),0,0,'C');
$pdf->SetX(123);
$pdf->Cell(58,5,utf8_decode(trim($usu[0]['strnombre'])).' '.utf8_decode(trim($usu[0]['strapellido'])),0,0,'C');
$pdf->Ln(5);
$pdf->SetX(25);
$pdf->SetLineWidth(.3);
...
if ($usu[0]["id_gerencia_maestro"]>=32 && $usu[0]["id_gerencia_maestro"]<=616) {
    $pdf->Row(array(utf8_decode($osti[0]['cargo']),'',utf8_decode($usu[0]['cargo'])));
}else if (($usu[0]['id_cargo_maestro']>=491 && $usu[0]['id_cargo_maestro']<=499)  || $usu[0]['id_cargo_maestro']==501 || $usu[0]['id_cargo_maestro']==505 || $usu[0]['id_cargo_maestro']==713 || $usu[0]['id_cargo_maestro']==763 || $usu[0]['id_cargo_maestro']==772 || $usu[0]['id_cargo_maestro']==875 || $usu[0]['id_cargo_maestro']==932){
    $pdf->Row(array(utf8_decode($osti[0]['cargo']),'',utf8_decode($usu[0]['cargo'])));
}else{
    $pdf->Row(array(utf8_decode($osti[0]['cargo']),'',utf8_decode($usu[0]['cargo']).' de la '.utf8_decode(trim($usu[0]['gerencia1']))));
}
$pdf->SetMargins(5,30,5);
if ($_REQUEST['gerencia']>='17' && $_REQUEST['gerencia']<='31'){
    $pdf->AddPage();
    $pdf->SetTextColor(0);
    $pdf->SetFont('Arial','B',10);
    $pdf->MultiCell(0, 5, strtoupper($_REQUEST['titulo']), 0, 'C');
    $pdf->Ln(10);
    ...
    $pdf->SetWidths(array(45,31,55,30,45));
    ...
    $sql="select b.*, g.stritema as gerencia, c.stritema as categoria,
		...
		and b.id_marca_maestro=m.id_maestro and b.id_estatus_maestro=e.id_maestro 
		and b.id_modelo_maestro=mo.id_maestro and b.bolborrado=0 and g.bolborrado=0 and c.bolborrado=0 
		and t.bolborrado=0 and m.bolborrado=0 and e.bolborrado=0 and mo.bolborrado=0  and u.bolborrado=0 and b.id_estatus_maestro in (86,88) and b.id_ubicacion in (3,4,680,681) and b.id_tipo_maestro<>132";
 		
 		if ($_REQUEST['gerencia']!=""){
			$sql.="AND b.id_gerencia_maestro='".$_REQUEST['gerencia']."' ";
		}
 		
    $sql.=" ORDER BY responsable, TIPO ASC";
    $conn= new Conexion();
    $conn->abrirConexion();
    $conn->sql=$sql;
    $data=$conn->ejecutarSentencia(2);
    ...
    if ($data){
        ...
        and b.bolborrado=0 and t.bolborrado=0 and b.id_gerencia_maestro='".$_REQUEST['gerencia']."' and b.id_estatus_maestro in (86,88) and b.id_ubicacion in (3,4,680,681) and b.id_tipo_maestro<>132
    ...
    $pdf->SetTextColor(255,255,255);
    $pdf->SetDrawColor(0);
    $pdf->SetWidths(array(40,35,60,40));
    $pdf->SetAligns(array('C', 'C', 'C', 'C'));
    $pdf->Row(array('EQUIPO','MARCA','MODELO','SERIAL'));
    $pdf->SetFont('Arial','',8);
    ...
    $sql="select b.*, g.stritema as gerencia, c.stritema as categoria,
        ...
        $sql.=" ORDER BY id_tipo_maestro ASC";
        $conn= new Conexion();
        $conn->abrirConexion();
        $conn->sql=$sql;
        $data=$conn->ejecutarSentencia(2);
    ...
    if ($data){
        for ($i= 0; $i < count($data); $i++){
            ...
            $pdf->SetTextColor(0);
            $pdf->SetAligns(array('L', 'L', 'L', 'L', 'L'));
            $pdf->Row(array(utf8_decode($data[$i]["tipo"]),utf8_decode($data[$i]["marca"]), utf8_decode($data[$i]["modelo"]),utf8_decode($data[$i]["strserial"]),));
        }
        ...
        if ($data1){
                ...
                for ($j= 0; $j < count($data1); $j++){
                        if ($equipo==$data1[$j]['equipo']){
                                ...
                                $totalc=$asignado ;
                                $ttotalc+=$totalc;
                                $pdf->Row(array(utf8_decode($equipo),$asignado));
