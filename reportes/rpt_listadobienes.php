...
$sql="select b.*, g.stritema as gerencia, c.stritema as categoria, 
        ...
        $sql.=" ORDER BY b.id_gerencia_maestro, b.id_unidad_maestro, b.id_responsable, b.id_tipo_maestro";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){
	if ($_REQUEST['continuo']!="1"){
	    ...
	    $pdf->Cell(0, 10,'Gerencia: '.$data[0]["gerencia"]." ".$data[0]["unidad"], 0, 0, 'L');
            $pdf->Ln();
		...
		for ($i= 0; $i < count($data); $i++){
			if ($data[$i]['responsable']=='DANADO') {
				...
				$pdf->SetDrawColor(0);
				$pdf->SetLineWidth(0.3);
				$pdf->Cell(0, 10,'Gerencia: '.$data[$i]["gerencia"]." ".$data[$i]["unidad"], 0, 0, 'L');
                                $pdf->Ln();
                                $pdf->SetFillColor(255,0,0);
                                $pdf->SetTextColor(255,255,255);
				$pdf->SetDrawColor(0);
	    		...
	    		switch ($campos) {
	    			...
	    			case 13:	
	    				...
	    				break;
                                case 14:	
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]],$data[$i][$nroCampos[9]],$data[$i][$nroCampos[10]],$data[$i][$nroCampos[11]],$data[$i][$nroCampos[12]],$data[$i][$nroCampos[13]]));
	    				break;
	    		}
