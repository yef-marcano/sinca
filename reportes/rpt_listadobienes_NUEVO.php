...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		//Logo
		$this->Image('../comunes/images/cintillo.jpg',10,3,200,0);
	...
	function Footer()
	{
		...
		if ($_REQUEST['filtro']!=""){
			$this->MultiCell(0,5,'Filtro Aplicado:'.$_REQUEST['filtro'],0,'J');
		}
...
$sql="select b.*, g.stritema as gerencia, c.stritema as categoria,
                case when b.id_direccion_maestro>0 then
                    (select stritema from tblmaestros where id_maestro=b.id_direccion_maestro and bolborrado=0)
                else
                    ''
                end as direccion,
                case when b.id_coordinacion_maestro>0 then
                    (select stritema from tblmaestros where id_maestro=b.id_coordinacion_maestro and bolborrado=0)
                else
                    ''
                end as coordinacion,
		...
		if ($_REQUEST['marca']!=""){
 		...
 		if ($_REQUEST['unidad']!=""){
			...
			$sql.="AND b.id_direccion_maestro in (".$_REQUEST['direccion'].") ";
		}
 		if ($_REQUEST['coordinacion']!=""){
			$sql.="AND b.id_coordinacion_maestro in (".$_REQUEST['coordinacion'].") ";
		}
        ...
        $sql.=" ORDER BY b.id_gerencia_maestro, b.id_direccion_maestro, responsable, t.stritema";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){
	if ($_REQUEST['continuo']!="1"){
	    ...
	    $gerencianombre=$data[0]["gerencia"]." ".$data[0]['direccion'];
	    $ubicacion= $data[0]["id_direccion_maestro"];
	    $pdf->SetFont('Arial','B',8);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0);
            $pdf->SetDrawColor(0);
            $pdf->SetLineWidth(0.3);
	    $pdf->Cell(0, 10,'Gerencia: '.$data[0]["gerencia"]." ".$data[0]["direccion"], 0, 0, 'L');
            $pdf->Ln();
		...
		for ($i= 0; $i < count($data); $i++){
			if ($data[$i]['responsable']=='DANADO') {
				...
				$responsable=$data[$i]['responsable'];
                        ...
                        if ($gerencianombre!=$data[$i]["gerencia"]." ".$data[$i]['direccion']){
				...
				if ($_REQUEST['consolidado']=="1"){
					...
					and b.bolborrado=0 and t.bolborrado=0 and b.id_gerencia_maestro='".$gerencia."' and b.id_direccion_maestro=".$ubicacion." ";
					if ($_REQUEST['nombre']!=""){
						$sql.="AND b.strnombre LIKE '%".strtoupper($_REQUEST['nombre'])."%' ";
					}
			 		if ($_REQUEST['serial']!=""){
					...
					$pdf->SetLineWidth(.3);
                                        switch ($cantidad) {
                                            case 1:
                                                ...
                                                $pdf->Row(array('Equipo',$estatus[0],'Total'));
                                                $pdf->SetAligns(array('L','R','R'));
                                                break;
                                            case 2:
                                                $pdf->SetWidths(array(40,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R'));
                                                break;
                                            case 3:
                                                $pdf->SetWidths(array(40,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R'));
                                                break;
                                            case 4:
                                                $pdf->SetWidths(array(40,23,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],$estatus[3],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R','R'));
                                                break;
                                            case 5:
                                                $pdf->SetWidths(array(40,23,23,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],$estatus[3],$estatus[4],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R','R','R'));
                                                break;
                                            case 6:
                                                $pdf->SetWidths(array(40,23,23,23,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],$estatus[3],$estatus[4],$estatus[5],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R','R','R','R'));
                                                break;
                                            case 7:
                                                $pdf->SetWidths(array(40,23,23,23,23,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],$estatus[3],$estatus[4],$estatus[5],$estatus[6],'Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R','R','R','R','R'));
                                                break;
                                            case 8:
                                                $pdf->SetWidths(array(40,23,23,23,23,23,23,23,23,20));
                                                $pdf->SetAligns(array('C','C','C','C','C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo',$estatus[0],$estatus[1],$estatus[2],$estatus[3],$estatus[4],$estatus[5],$estatus[6],$estatus[7],'Total'));
				...
				$pdf->SetDrawColor(0);
				$pdf->SetLineWidth(0.3);
				$pdf->Cell(0, 10,'Gerencia: '.$data[$i]["gerencia"]." ".$data[$i]["direccion"], 0, 0, 'L');
                                $pdf->Ln();
                                $pdf->SetFillColor(255,0,0);
                                $pdf->SetTextColor(255,255,255);
				$pdf->SetDrawColor(0);
	    		...
	    		switch ($campos) {
	    			case 1:
	    				$pdf->Row(array($data[$i][$nroCampos[0]]));
	    				break;
	    			case 2:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]]));
	    				
	    				break;
	    			case 3:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]]));
	    				break;
	    			case 4:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]]));
	    				break;
	    			case 5:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]]));
	    				break;
	    			case 6:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]]));
	    				break;
	    			case 7:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]]));
	    				break;
	    			case 8:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]]));
	    				break;
	    			case 9:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]]));
	    				break;
	    			case 10:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]],$data[$i][$nroCampos[9]]));
	    				break;
	    			case 11:
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]],$data[$i][$nroCampos[9]],$data[$i][$nroCampos[10]]));
	    				break;    				
	    			case 12:	
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]],$data[$i][$nroCampos[9]],$data[$i][$nroCampos[10]],$data[$i][$nroCampos[11]]));
	    				break;
	    			case 13:	
	    				$pdf->Row(array($data[$i][$nroCampos[0]],$data[$i][$nroCampos[1]],$data[$i][$nroCampos[2]],$data[$i][$nroCampos[3]],$data[$i][$nroCampos[4]],$data[$i][$nroCampos[5]],$data[$i][$nroCampos[6]],$data[$i][$nroCampos[7]],$data[$i][$nroCampos[8]],$data[$i][$nroCampos[9]],$data[$i][$nroCampos[10]],$data[$i][$nroCampos[11]],$data[$i][$nroCampos[12]]));
	    				break;
	    		}
			...
			$ubicacion=$data[$i]["id_direccion_maestro"];
			$gerencianombre=$data[$i]["gerencia"]." ".$data[$i]['direccion'];
		}
		$pdf->SetWidths(array(252));
	    $pdf->SetAligns(array('R'));
	    $pdf->Row(array('Total de Equipos '.$total));
					...
					$pdf->SetLineWidth(.3);
                                        switch ($cantidad) {
                                            ...
                                            default:
                                                $pdf->SetWidths(array(40,20,20,20,20,20,25,20,25,25));
                                                $pdf->SetAligns(array('C','C','C','C','C','C','C','C','C','C'));
                                                $pdf->Row(array('Equipo','Disponible','Asignado',utf8_decode('Pr̩stamo'),utf8_decode('Da̱ado'),'Extraviado','Desincorporado','Robado','Reemplazado','Total'));
                                                $pdf->SetAligns(array('L','R','R','R','R','R','R','R','R','R'));
