...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		if ($_REQUEST['titulo']!=""){
			...
			$this->Cell(0,10,utf8_decode('LISTADO TELÉFONOS CORPORATIVOS'),0,0,'C');
		}
		$this->Ln(10);
...
pr.strdescripcion as proveedor,tl.strdescripcion as telefono,
g.stritemb as gerencia1
from tblbienes b, tblmaestros g,tblmaestros t, 
tblmaestros mo,  tblmaestros m, tbldetallebienes pr, tbldetallebienes tl
where b.id_gerencia_maestro=g.id_maestro 
and b.id_estatus_maestro=t.id_maestro
and b.id_marca_maestro=m.id_maestro
and b.id_bienes=pr.id_bienes and b.id_bienes=tl.id_bienes
and pr.id_caracteristica_maestro=434  and tl.id_caracteristica_maestro=433
and pr.bolborrado=0 and tl.bolborrado=0 
and b.id_modelo_maestro=mo.id_maestro and b.bolborrado=0 and g.bolborrado=0 
and t.bolborrado=0 and mo.bolborrado=0  and m.bolborrado=0  and b.id_tipo_maestro=132
...
and b.id_marca_maestro=m.id_maestro
and b.id_bienes=pr.id_bienes and b.id_bienes=tl.id_bienes
and pr.id_caracteristica_maestro=434  and tl.id_caracteristica_maestro=434
and pr.bolborrado=0 and tl.bolborrado=0
and b.id_modelo_maestro=mo.id_maestro and b.bolborrado=0 and g.bolborrado=0
and t.bolborrado=0 and mo.bolborrado=0  and m.bolborrado=0  and b.id_tipo_maestro=132
and tl.strdescripcion LIKE '%".strtoupper($_REQUEST['proveedor'])."%'";
	...
	mo.stritema as modelo,
	case when (select d.id_detallebienes from tbldetallebienes d where d.bolborrado=0 and d.id_bienes=b.id_bienes and d.id_caracteristica_maestro=434)>0 then
		(select d.strdescripcion from tbldetallebienes d
		where d.bolborrado=0 and d.id_bienes=b.id_bienes and d.id_caracteristica_maestro=434)
	else
	
		'SE DESCONOCE'
	end as proveedor,
	
	case when (select d.id_detallebienes from tbldetallebienes d where d.bolborrado=0 and d.id_bienes=b.id_bienes and d.id_caracteristica_maestro=433)>0 then
		(select d.strdescripcion from tbldetallebienes d
		where d.bolborrado=0 and d.id_bienes=b.id_bienes and d.id_caracteristica_maestro=433)
	else
	
		'N/A'
	end as telefono, g.stritemb as gerencia1
	from tblbienes b, tblmaestros g,tblmaestros t, 
...
$sql.=" order by proveedor,responsable";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){
	...
	for ($i= 0; $i < count($data); $i++){
		...
		if ($proveedor!=$data[$i]["proveedor"]){
			...
			$pdf->Row(array(utf8_decode('Total de Teléfonos '.$total)));
            $pdf->Ln();    
			$total=0;
			$pdf->AddPage();
			$pdf->SetFont('Arial','B',8);
			...
			$pdf->SetDrawColor(0);
			$pdf->SetLineWidth(0.3);
			$pdf->Cell(0, 10,'Proveedor: '.$data[$i]["proveedor"], 0, 1, 'L');     
            $pdf->Ln();
            $pdf->SetFillColor(255,0,0);
			$pdf->SetTextColor(255,255,255);
			$pdf->SetDrawColor(0);
    ...
    if ($_REQUEST['linea']==""){
            if ($_REQUEST['proveedor']!=""){
                ...
                and b.bolborrado=0 and t.bolborrado=0  and b.id_bienes=tl.id_bienes and tl.bolorrado=0
                and b.id_tipo_maestro=132 and tl.id_caracteristica_maestro=434 and b.bolborrado=0
                and tl.strdescripcion like '%".strtoupper($_REQUEST['proveedor'])."%'";
            }else{
                    $sql="select count(b.*),
                t.stritema as estatus,b.id_estatus_maestro,
