...
$fname = tempnam("/tmp", "telefonos.xls");
$libro = &new writeexcel_workbookbig($fname);
$hoja = &$libro->addworksheet('TELEFONOS CORPORATIVOS');
...
if ($_REQUEST['titulo']!=""){
    ...
    $hoja->write(0,4,utf8_decode('LISTADO TELÉFONOS CORPORATIVOS'),$lo_encabezado);
}
...
$hoja->write(2,6,utf8_decode('CARGO'),$lo_titulo);
$hoja->write(2,7,'GERENCIA',$lo_titulo);
$hoja->write(2,8,'RESPONSABLE',$lo_titulo);
$hoja->write(2,9,utf8_decode('ESTATUS'),$lo_titulo);
$hoja->write(2,10,utf8_decode('OBSERVACIÓN'),$lo_titulo);
$sql="select b.*,
...
if ($data){
    for ($i= 0; $i < count($data); $i++){
        ...
        $hoja->write($i+3, 3,utf8_decode($data[$i]["marca"]),$lo_dataleft);
        $hoja->write($i+3, 4,utf8_decode($data[$i]["modelo"]),$lo_dataleft);
        $hoja->write($i+3, 5,utf8_decode($serial),$lo_dataleft);
        $hoja->write($i+3, 6,utf8_decode($data[$i]["cargo"]),$lo_dataleft);
        $hoja->write($i+3, 7,utf8_decode($data[$i]["gerencia"]),$lo_dataleft);
        $hoja->write($i+3, 8,utf8_decode($data[$i]["responsable"]),$lo_dataleft);
        $hoja->write($i+3, 9,utf8_decode($data[$i]["estatus"]),$lo_dataleft);
        $hoja->write($i+3, 10,utf8_decode($data[$i]["memobservacion"]),$lo_dataleft);
    ...
    header("Content-Type: application/x-msexcel; name=\"telefonos.xls\"");
    header("Content-Disposition: inline; filename=\"telefonos.xls\"");
    $fh=fopen($fname, "rb");
    fpassthru($fh);
    unlink($fname);
}else{
