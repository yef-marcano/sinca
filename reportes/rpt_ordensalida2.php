...
if($data){
	...
	", titular de la Cédula de Identidad: ".number_format($data[0]['cedula_usuario'],0,",",".").", a retirar de la Sede Fondemi ".$data[0]['sede'].
	" los siguientes equipos que se describen a continuación:");
	...
	$recibe=$usuario->selectUsuarioById($data[0]['id_autorizado']);
        ...
        if ($data1){
            for ($i=0; $i<count($data1); $i++){	
                ...
                $pdf->SetFont('Arial','',8);
                $pdf->SetAligns(array('C', 'L','L','L', 'C'));
                $pdf->Row(array(1,utf8_decode($data1[$i]['tipo']),utf8_decode($data1[$i]['marca']),utf8_decode($data1[$i]['modelo']), $data1[$i]["strserial"]));
            }
	}
       
	$pdf->Ln(5);
	$pdf->SetFont('Arial','',9);
	$pdf->MultiCell(0,8,utf8_decode('OBSERVACIÓN: ').utf8_decode($data[0]['memobservacion']),0,'J',false);
	$pdf->Ln(30);
	$pdf->SetFont('Arial','B',11);
	$pdf->SetX(70);
	...
	$pdf->Ln(8);	
	$pdf->SetX(70);
	$pdf->MultiCell(85,5,utf8_decode($recibe[0]['cargo']),0,'C');
