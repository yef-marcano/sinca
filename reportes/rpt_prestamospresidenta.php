...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,utf8_decode('PRÉSTAMOS REALIZADOS POR PRESIDENCIA'),0,0,'C');
		$this->Ln(10);
...
if ($data){
	...
	$pdf->Row(array('EQUIPO','MARCA','MODELO','SERIAL','USUARIO','FECHA',utf8_decode('OBSERVACIÓN')));
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["unidad"]){
            ...
            $pdf->Row(array(utf8_decode($data[$i]["tipo"]),utf8_decode($data[$i]["marca"]), utf8_decode($data[$i]["modelo"]),utf8_decode($data[$i]["strserial"]), $responsable,$fecha,utf8_decode($data[$i]['memobservacion'])));
			$total++;
		}
		else
		{
    ...
    $pdf->Ln(20);   
    $data="";
    $sql="select count(b.*),b.id_unidad_maestro, u.stritema as unidad, t.stritema as equipo
...
group by b.id_unidad_maestro, u.stritema, t.stritema
order by b.id_unidad_maestro, u.stritema, t.stritema";
    $conn= new Conexion();
	$conn->abrirConexion();
	$conn->sql=$sql;
	$data=$conn->ejecutarSentencia(2); 
