...
class PDF  extends PDF_MC_Table
{
	function Header()
	{
		...
		$this->Cell(0,10,'PR�STAMOS REALIZADOS POR PRESIDENCIA',0,0,'C');
		$this->Ln(10);
...
$sql="select b.*, g.stritema as gerencia, c.stritema as categoria, 
        ...
        $sql.=" ORDER BY b.id_gerencia_maestro, b.id_unidad_maestro, t.stritema, responsable";
$conn= new Conexion();
$conn->abrirConexion();
$conn->sql=$sql;
...
if ($data){
	...
	$pdf->SetDrawColor(0);
    $pdf->SetWidths(array(40,40,40,50,50,30));
	$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C','C'));
	$pdf->Row(array('EQUIPO','MARCA','MODELO','SERIAL','USUARIO','FECHA'));
	for ($i= 0; $i < count($data); $i++){
    	...
    	if ($gerencia!=$data[$i]["unidad"]){
            ...
            $pdf->Row(array(utf8_decode($data[$i]["tipo"]),utf8_decode($data[$i]["marca"]), utf8_decode($data[$i]["modelo"]),utf8_decode($data[$i]["strserial"]), utf8_decode($responsable),$fecha));
			$total++;
		}
		else
		{
